...
const VendorProductsPage = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      const response = await fetch(`${API_BASE}/vendors/products`, { headers: authHeaders() });
    ...
    <div className="mx-auto max-w-6xl px-6 py-16 lg:px-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-3 text-slate-600">Manage your inventory and product listings directly from PINAKK.</p>
        </div>
        <Link href="/vendors/products/new" className="inline-flex items-center justify-center rounded-full bg-secondary px-6 py-3 text-sm font-semibold text-white hover:bg-orange-600">
          Add new product
        </Link>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-200">
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="px-6 py-6 text-center text-sm text-slate-500">No vendor products yet.</td>
              </tr>
            )}
          </tbody>
        </table>
